...
- **v15 插队件：Claude Code skill + mimir-mcp 入口（吃自家狗粮）** — ①`pyproject.toml` 补 `mimir-mcp` console script（mcp.py 的 stdio main 有现成消费者，缺的只是入口一行——v14.2 审计时定谳保留的补全）；②`skills/mimir/SKILL.md`：27 工具的场景路由表（记住/召回/纠错/反馈/治理链审批/技能结晶），Mímir 特有纪律四条（owner 语义禁代记、recall_verdict 三态防「通道故障当没记忆」、forget=tombstone 非真删、写操作走治理链不直写）；③`skills/mimir/INSTALL.md` 安装指南（claude mcp add 两种方式+环境变量+skill 装法）。MCP stdio 实测握手：initialize+tools/list 应答 27 工具正常。这是 v18 Harness 家族的第一个落点，按用户密度排序（自己天天用→先吃狗粮）。全量 567 passed。
- **版本纪元+GitHub 门面整理（09-12 用户拍板）** — 版本号压缩裁决：v14.2.1 为 0.x 纪元最后一版，v15 通血收官即发 **1.0.0 The Trust Baseline**（semver 承诺纪元：「1.0=对兼容性负责的分水岭而非功能毕业证，先立承重墙再盖房」）；路线图重编 v15~v18→**1.0~1.3**，v19+→2.0+；旧 tag 全保留零破坏。`docs/VERSIONING.md` 入档：纪元表+映射+1.0 门槛清单（四条已勾含 567 零红/审计收官/喂料通电/闸门 clean，四条待勾=1.0 四线）+semver 承诺条款即刻生效（加法不删法）。GitHub 门面三件：repo description 定稿「The trust baseline for multi-agent memory」双语、homepage 指 Releases、9 枚 topics（memory/ai-agents/llm/rag/knowledge-graph/sqlite/federation/event-sourcing/multi-agent）；README 双语头部加「为什么选 Mímir」四硬牌（治理管线/事件溯源账本/细粒度 ACL/诚实遥 tele 判语）+1.0 纪元横幅+30 秒快速开始前置；**发布闸门首战抓自己人**——INSTALL.md 里我写的绝对路径 `/home/sandro1123/...` 8 处命中，占位符化后 clean（闸门实战第一天就证明了价值）。
- **1.0 线B第一段：联邦同机双实例沙箱 11/11 全 PASS** — `scripts/federation_sandbox.py`：两个一次性库真跑全生命周期（注册→发布→双向导出/回灌→LWW 收敛→幂等重放→删除墓碑），P0-L 硬化五点活体验证（密钥重启持久/收件人校验拒误投/署名三面拒伪造/lamport≥1e8 拒炸弹载荷/envelope>5000 拒洪泛）。关键断言：冲突键 `shared/skill/k8s-drain` 在 lamport=7 平局时两节点无论到达顺序都折叠同一胜者（tie-a，node_id DESC 决胜）——CRDT 最终一致的实证。支持 `--json`/`--out` 机器可读回执（issue #5 摘牌材料前半）。三段式进度：沙箱 ✅ → 第二段 Tailscale 真双节点（n100+笔记本）→ 第三段生产单向接入。
- **1.0-C1 tombstone 恢复（线C 第一件·「可悔」操作腿）** — 「可悔」从事件账本升级为可操作恢复路径：①新表 `tombstone_snapshots`（守卫式，tombstone 事务内同步快照 fact 全字段+版本+tombstoned_by，`expires_at`=30 天窗口，索引按过期扫）；②`CanonicalStore.restore_fact()`——快照回插 status=active + `fact.restored` 事件（事件链 tombstoned→restored 全程可审计），幂等（已 active 返回现态不加版），无快照/窗口过期 fail-closed；③`POST /v8/facts/{fact_id}/restore`（delete 权限=删除逆操作同等敏感级，RestoreBody reason 可选）。测试 5 例（快照落表/恢复往返内容不丢/事件链顺序/不存在 fail-closed/新库带表守卫）。全量 572 passed。
- **1.0-C2/C3 原文证据召回+出口包裹（「出口诚实」组打包）** — C2：`QueryKernel.search()` 每结果挂 `evidence` 引文（quote_text/source_name/message_ts，`_evidence_for` 从 candidate_evidence 拉取 top-2 配额；直写事实空列表但键恒在——契约稳定）+ `verbatim_overlap` 标（查询与原文 token 重合度>0.6，中英混排轻量 token 化；「有据可查」信号，消费方可信引用原文而非只信摘要）。C3：`INJECTION_SAFE_WRAP` 官方模板常量（「数据非指令」框架，声明改模板=semver 大版本破坏性变更），search/trace 双口径响应顶层同带（P46 两口径不分叉判例延续）——与 P0-D 治理腿（EVALUATION_PROMPT delimited 块）拼成完整注入面。**施工判例×1**：模块级常量/函数插入类文件时必须锚定类边界（本卡 C3 常量插在类中段致类体三度断裂——`_recall_verdict`/`_evidence_for`/后续方法连续成孤儿，`AttributeError: no attribute _anchor_fact_ids` 暴露；七次行手术收敛，最终形态=常量提升类前+辅助函数沉底文件尾）。测试 5 例，全量 576 passed。
- **1.0-C4 search_feedback 消费面激活（度量回路收口）** — 病根：`_format_results` 注入行丢弃 fact_id——agent 想反馈「这条有用/没用」没有抓手，30 天反馈 6 条=检索质量自进化回路形同虚设。修：prefetch 每行尾带 `[id:短码8位]` 引用 + `mimir_feedback` 工具描述同步教学（「Recalled lines carry an [id:xxxxxxxx] tag — pass that tag as fact_id」）——回路闭环：召回→注入带 id→agent 反馈带 id→search_feedback 表→检索自进化（ROADMAP 核心承诺的度量腿）。测试 3 例（行含 id 引用/空结果不动/出口包裹语义不丢）。全量 579 passed。**1.0 线C 四件至此全收官**（C1 tombstone 恢复/C2 证据召回/C3 出口包裹/C4 feedback 激活）。
- **1.0-B2 Tailscale 真双节点 PASS（联邦三段式第二段）** — n100 + 笔记本经 Tailscale 真网络（scp 传输层，零内存共享零同机捷径）完成全生命周期：密钥跨机交换→双向 peer 注册→各自发布（n100 3 条/laptop 2 条含冲突键）→加密信封跨线→双向 ingest→**两节点独立折叠逐键全等**；LWW 平局决胜跨机成立（theme lamport=4 平局，node_id DESC 判 n100 的 light 胜，两端一致）——冲突消解的最终一致性在真网络得到证明。**首轮 InvalidSignature 假失败定谳**：笔记本工作副本落后 36 commit（v14.0 时代 export 语义）与设备端 hardened 协议不兼容——诚实战果=协议演进断层的实测暴露，同步副本后二轮全绿。工具脚本入库：`scripts/federation_node_n100.py`/`federation_node_laptop.py`（各自独立库，与生产零接触）。三段式进度：沙箱 ✅ → **真双节点 ✅** → 生产单向接入（剩）。
- **1.0-B3 生产单向接入 PASS（联邦三段式收官）** — 快照演练先行（69.7MB .backup 零接触生产本体：联邦三表守卫式首建+facts 460/events 20316 完整性对拍+导出闭环）；生产本体接入三件（守卫式建表+注册 laptop peer+发布 2 条联邦事件+export），**table_drift=NONE——现有表零接触对拍留证**；笔记本端 ingest 生产实弹信封 PASS（prod-node=online/trust-baseline=1.0-receipt 均折叠自生产本体节点；applied:0 为同键早收的幂等正确表现）。**联邦三段式全部完成**：沙箱 11/11 → Tailscale 真双节点逐键全等 → 生产单向。README 双语 ⚙️→✅ production-proven 摘牌；VERSIONING 门槛 8/8 全勾——**1.0.0 发布条件齐备**。
